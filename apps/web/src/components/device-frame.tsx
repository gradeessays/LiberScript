...
export function DeviceFrame({
  ...
  const screen = (
    <iframe
      ...
      style={{ width: s.w, height: s.h, border: 0, display: 'block', background: '#fff', borderRadius: s.screenRadius }}
    />
  );
  ...
  const chassis =
    device === 'pc' ? (
      <div style={{ width: outerW }}>
        <div style={{ width: outerW, background: '#111', borderRadius: s.bodyRadius, padding: s.bezel, boxShadow: '0 14px 50px rgba(0,0,0,0.3)', boxSizing: 'border-box' }}>
          <div style={{ height: s.chrome, background: '#2a2a2c', borderRadius: '8px 8px 0 0', display: 'flex', alignItems: 'center', gap: 6, padding: '0 12px' }}>
      ...
      <div style={{ position: 'relative', width: outerW, background: '#0b0b0c', borderRadius: s.bodyRadius, padding: s.bezel, boxShadow: '0 14px 50px rgba(0,0,0,0.35)', boxSizing: 'border-box' }}>
        {s.notch && (
          <div style={{ position: 'absolute', top: s.bezel, left: '50%', transform: 'translateX(-50%)', width: 130, height: 24, background: '#0b0b0c', borderRadius: '0 0 14px 14px', zIndex: 2 }} />
        )}
        {s.camera && (
          <div style={{ position: 'absolute', top: s.bezel / 2 - 3, left: '50%', transform: 'translateX(-50%)', width: 7, height: 7, borderRadius: 99, background: '#3a3a3d' }} />
        )}
        {screen}
      </div>
    );
  ...
  return (
    <ScaledStage width={outerW} height={outerH} maxHeight={maxHeight}>
      {chassis}
    </ScaledStage>
  );
}
